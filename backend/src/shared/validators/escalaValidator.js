...
const criarEscalaSchema = escalaBase.refine(timeRefine, { message: 'hora_fim deve ser maior que hora_inicio' });

const atualizarEscalaSchema = escalaBase.partial().refine((data) => {
  if (!data.hora_inicio || !data.hora_fim) return true;
  return timeRefine(data);
