...
class HorarioAulaRepository {
  async listar({ limite, offset, accountId }) {
    ...
    const consulta = `SELECT * FROM horarios_aula WHERE account_id = ? ORDER BY dia_semana ASC, hora_inicio ASC LIMIT ? OFFSET ?`;
    const params = [accountId, limite, offset];

    const [dados] = await conexao.query(consulta, params);
    const [count] = await conexao.query('SELECT COUNT(*) AS total FROM horarios_aula WHERE account_id = ?', [accountId]);
